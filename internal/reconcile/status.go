        ...
        WHERE current_jobs.payer_id = r.payer_id
          ...
          AND (current_jobs.job_id IS NULL OR NOT EXISTS (
              ...
              WHERE j.id = current_jobs.job_id
                ...
                AND j.args = jsonb_build_object(current_jobs.arg_key, to_jsonb(current_jobs.domain_id))
          ))
    )
) blockers
...
WHERE id = :source_id`
	...
	// SQLStaleStages is diagnostic only. The interval bind is PostgreSQL's
	// positive interval threshold (for example, '30 minutes').
	SQLStaleStages = `
...
WHERE o.age_from <= CURRENT_TIMESTAMP - $1::interval
