...
// Runtime is the production River process for Stories 06–07.
type Runtime struct {
	Pool       *pgxpool.Pool
...
// Queues is the Story 07 worker map: discovery, toc_download, and toc_parse.
...
// Run starts a River client that consumes discovery.run, toc.download, and
// toc.parse, waits until ctx is canceled or the client stops, then shuts
// down. A requested shutdown after Start succeeds returns nil.
